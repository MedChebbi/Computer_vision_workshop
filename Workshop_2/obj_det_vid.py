...
# load the DNN model
model = cv2.dnn.readNet(model=weightsPath,
                        config=configPath, 
                        framework='TensorFlow')

# capture the video
cap = cv2.VideoCapture('../resources/videos/video_1.mp4')
# get the video frames' width and height for proper saving of videos
frame_width = int(cap.get(3))
frame_height = int(cap.get(4))
# create the `VideoWriter()` object
out = cv2.VideoWriter('../resources/videos/video_result.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, 
    ...
    if ret:
        ...
        for detection in output[0, 0, :, :]:
            ...
            if confidence > .4:
                ...
                cv2.putText(image, class_name, (int(box_x), int(box_y - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
