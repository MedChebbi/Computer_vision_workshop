...
with open('../resources/models/classification_classes_ILSVRC2012.txt', 'r') as f:
    image_net_names = f.read().split('\n')
# final class names (just the first word of the many ImageNet names for one image)
class_names = [name.split(',')[0] for name in image_net_names]

# load the neural network model
model = cv2.dnn.readNet(model='../resources/models/DenseNet_121.caffemodel', 
                      config='../resources/models/DenseNet_121.prototxt', 
                      framework='Caffe')

# load the image from disk
image = cv2.imread('../resources/images/tiger.jpg')
...
cv2.waitKey(0)
cv2.imwrite('../resources/images/result_image.jpg', image)
